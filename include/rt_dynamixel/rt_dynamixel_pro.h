...
//				delete[] pucTable;
    ...
    // Editor : PARK Gyeong Jae (rudwo1301@snu.ac.kr)
    ...
    public:
        pthread_mutex_t rtDynamixelMutex;
        pthread_t rtDynamixelThread;
        long rttLoopStartTime;         ///< Control loop start time.
        long rttLoopTimeoutTime;       ///< Control loop timeout time.
            ...
            rt_dynamixel(), nIndex(index), bControlLoopEnable(false), bControlWriteEnable(false)
        {
            ...
            //pthread_mutex_init(&rtDynamixelMutex,NULL);
        ...
        {
            printf("destroy\n");
            pthread_cancel(rtDynamixelThread);
            //rt_task_delete(&rttTaskObject);
            pthread_mutex_destroy(&rtDynamixelMutex);
            //rt_mutex_delete(&rttDataMutex);
        }
        void mutex_acquire()
        {   
         //   pthread_mutex_lock(&rtDynamixelMutex);
            //rt_mutex_acquire(&rttDataMutex,0);
        }

        void mutex_release()
        {   
          //  pthread_mutex_unlock(&rtDynamixelMutex);
        ...
        bool IsPacketTimeout()
        {
            ...
            if (temp.tv_nsec > rttLoopTimeoutTime)
            { return true; }
            return false;
        }
        ...
        { 
            ...
            pthread_create(&rtDynamixelThread, &attr, &dxl_control, this);
            pthread_detach(rtDynamixelThread);
